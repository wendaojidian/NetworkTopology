...
def read_line(conf_str):
    ...
    '''
    lines = conf_str.replace('\r','').split('\n')
    res_line = ""
    for line in lines:
        # 处理空行，空行只有一个换行符
        if len(line) == 0:
...
def check_conf_file(conf_str):
    ...
    for line in lines:
        ...
        #     print(ascii(l))
        # print(line=='Apache')
        # print(line, line, line == 'Apache')
        # print(line, line == "Apache")
        if line == "Apache":
            continue
        if line.find('=') == -1:
            return False
...
if __name__ == '__main__':
    ...
    raw_conf=get_raw_conf("./conf.yml")
    # print(raw_conf)

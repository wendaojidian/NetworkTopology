...
from .views.topologyconfig import TopologyConfigLines, TopologyConfigScript, TopologyConfigList
...
urlpatterns = [
    ...
    # 输入配置文件内容，返回命令列表
    path('config-lists', TopologyConfigScript.as_view()),
    # 执行命令列表的单条命令
    path('config-list', TopologyConfigList.as_view()),
    # 执行单条命令
    path('config-lines', TopologyConfigLines.as_view()),
    # 获取拓扑图
